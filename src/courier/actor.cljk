(ns courier.actor
  "CourierRouteActor — the ISCO-08 9621 messenger/package-deliverer/
  luggage-porter delivery-route scheduling/logistics coordination actor
  as a `langgraph.graph/state-graph` (ADR-2607121000 / CLAUDE.md Actors
  ...
  cloud-itonami-isco-9611's wastecollect.actor for the outdoor-route
  hazard-domain shape, extended with a second, independent
  delivery-traffic/manual-lifting hazard-scope dimension.
  ...
  The unconditional invariant: the Messenger, Package Deliverer and
  Luggage Porter Advisor can never directly commit an operating record
  the CourierRouteGovernor refuses, and can never make this actor
  finalize a delivery-execution decision (e.g. authorizing a specific
  delivery route/luggage-handling operation) or a route-safety-
  clearance decision (e.g. declaring a route cleared for safety), and
  can never override a route safety supervisor's judgment — every
  commit-record! call is gated behind `:decide`, and both scope-excluded
  decision classes are a permanent hard block regardless of disposition
  path. This actor coordinates ROUTE SCHEDULING/LOGISTICS ONLY — it
  never performs delivery or portering work itself and never makes a
...
(defn build-graph
  "Build a compiled CourierRouteActor graph. `store` implements
  `courier.store/Store`. `advisor` implements `courier.advisor/Advisor`
